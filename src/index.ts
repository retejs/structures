...
type Structures<N extends BaseN, C extends BaseC> = {
    ...
    successors(id: NodeId, selector?: (node: N, con: C) => boolean): Structures<N, C>
...
export function structures<N extends BaseN, C extends BaseC>(context: Context<N, C>): Structures<N, C> {
  ...
  function outgoers(id: NodeId, selector: (node: N, connection: C) => boolean = Boolean) {
    const nodes = data.connections
      .filter(c => {
  ...
  function collectSuccessors(id: NodeId, set: Set<N['id']>, selector: (node: N, con: C) => boolean) {
  ...
  function successors(id: NodeId, selector: (node: N, con: C) => boolean = Boolean) {
  ...
  function getChildren(selector: (node: N) => boolean = Boolean) {
  ...
  function descendants(selector: (node: N) => boolean = Boolean) {
